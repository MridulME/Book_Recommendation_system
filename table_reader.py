...
def connect_to_mysql():
    ...
    try:
        ...
        return book_df, user_df, book_query, user_query

    except SQLAlchemyError as e:
        logging.error(f"Error connecting to the database: {e}")
        return None, None, None, None

def refresh_data(book_query, user_query):
    """
...
def start_background_refresh(book_df, user_df, book_query, user_query, interval=60):
    """
    Continuously checks for updates to the DataFrames at the specified interval.
    """
    def check_for_updates():
        nonlocal book_df, user_df  # Allow the function to modify the outer scope variables
        while True:
            try:
                new_book_df, new_user_df = refresh_data(book_query, user_query)
